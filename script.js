...
// 2. THE MAZE (35 Nodes - Solvable via Bottom Left)
const nodes = [
    // Top Row
...
let enemies = [
    { id: 0, currentNode: 25, animX: 0, animY: 0, startNode: 25, type: 'gian', catchPhrase: "Gian: 'Hahaa! You think you're slick? Caught you! Come to my concert now!'" },
    { id: 1, currentNode: 10, animX: 0, animY: 0, startNode: 10, type: 'suneo', catchPhrase: "Suneo: 'My strategy is perfect! You can't outsmart me, Nobita!'" },   
    { id: 2, currentNode: 22,  animX: 0, animY: 0, startNode: 22,  type: 'sensei', catchPhrase: "Sensei: 'Nobita! Where are you running to after skipping school?! Detention for you!'" }
];
...
function draw() {
    ...
    ctx.save();
    ctx.shadowColor = COLORS.goal; ctx.shadowBlur = 15;
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (clicked) {
        if (nodes[playerNode].neighbors.includes(clicked.id)) handleMove(clicked.id);
        else if (clicked.id !== playerNode) showDialog("Nobita", "Too far! I can only move to red circles.");
    }
});
...
canvas.addEventListener('touchstart', (e) => {
    ...
    if (clicked) {
        if (nodes[playerNode].neighbors.includes(clicked.id)) handleMove(clicked.id);
        else if (clicked.id !== playerNode) showDialog("Nobita", "Too far!");
    }
}, { passive: false });
...
function startGame() {
    ...
    isPlaying = true; resetLevel(); requestAnimationFrame(gameLoop);
}

function resetLevel() {
    playerNode = 0; turn = 0; turnCount.innerText = 0;
...
// --- UPDATED GAME OVER WITH PORTRAITS ---
...
window.startGame = startGame; window.resetLevel = resetLevel; window.closeDialog = closeDialog;
setupCanvas();
